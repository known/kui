function DevCode() {
    ...
    var tabs = new Tabs({
        ...
        items: [
            ...
            { name: '前端代码', component: '<pre id="codeView" class="code"></pre>' },
            { name: '实体代码', component: '<pre id="codeEntity" class="code"></pre>' },
            { name: '控制器代码', component: '<pre id="codeController" class="code"></pre>' },
            { name: '业务逻辑层', component: '<pre id="codeService" class="code"></pre>' },
            { name: '数据访问层', component: '<pre id="codeRepository" class="code"></pre>' },
            { name: '数据库脚本', component: '<pre id="codeSql" class="code"></pre>' }
        ]
    });

    //methods
    this.render = function (dom) {
        Utils.addJs(baseUrl + '/static/libs/prettify/prettify.js');
        Utils.addJs(baseUrl + '/static/libs/prettify/lang-css.js');

        var fit = $('<div class="fit">').appendTo(dom);
        var left = $('<div class="fit-col-3">').appendTo(fit);
        tree.render().appendTo(left);
        var right = $('<div class="fit-col-7">').appendTo(fit);
...
function DevCodeForm(tree) {
    ...
    var grid = new Grid('Field', {
        ...
        columns: [
            {
                action: 'add', icon: 'fa fa-plus', align: 'center', width: '60px', actionFormat: function (d) {
                    return '<span class="link red fa fa-trash-o" action="remove" title="删除"></span>'
                        + ' <span class="link fa fa-arrow-up" action="up" title="上移"></span>'
                        + ' <span class="link fa fa-arrow-down" action="down" title="下移"></span>';
                }
            },
    ...
    this.render = function () {
        var elem = $('<div class="fit">');
        var tb = $('<div style="padding:5px;">').appendTo(elem);
        _initToolbar(tb);
