    ...
    function _setQuery(querys) {
        ...
        for (var i = 0; i < querys.length; i++) {
            ...
            if (q.type === 'dateRange') {
                ...
                var start = new Input(item, $.extend({}, q, { type: 'date', id: 'start' + q.field, value: sDate.format('yyyy-MM-dd') }));
                _setInput(start);
                var end = new Input(item, $.extend({}, q, { type: 'date', id: 'end' + q.field, value: now.format('yyyy-MM-dd') }));
                _setInput(end);
            } else {
