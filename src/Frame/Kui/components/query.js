function Query(option) {
    ...
    function _setQuery(querys) {
        ...
        for (var i = 0; i < querys.length; i++) {
            ...
            var item = $('<div class="query-item">').appendTo(_elem);
            ...
            if (qi.type === 'dateRange') {
                ...
                var start = new Input(item, $.extend({}, qi, { type: 'date', field: sPre + qi.field, placeholder: 'yyyy-MM-dd' }));
                _setInput(start);
                start.elem.fdatepicker({ format: 'yyyy-mm-dd', initialDate: sDate.format('yyyy-MM-dd'), endDate: now });
                $('<span>~</span>').appendTo(item);
                var ePre = qi.prefixType === 'LG' ? 'G' : 'end';
                var end = new Input(item, $.extend({}, qi, { type: 'date', field: ePre + qi.field, placeholder: 'yyyy-MM-dd' }));
                _setInput(end);
                end.elem.fdatepicker({ format: 'yyyy-mm-dd', initialDate: now.format('yyyy-MM-dd'), endDate: now });
            } else if (qi.type === 'dateMonth') {
                var input = new Input(item, $.extend({}, qi, { type: 'date', placeholder: 'yyyy-MM' }));
                _setInput(input);
                ...
                var input = new Input(item, $.extend({}, qi, { type: 'date', placeholder: 'yyyy-MM-dd' }));
                _setInput(input);
                input.elem.fdatepicker({ format: 'yyyy-mm-dd', initialDate: now.format('yyyy-MM-dd'), endDate: now });
        ...
        Utils.createButton({
            style: 'primary', icon: 'fa fa-search', text: Language.Search, handler: function () {
                _search();
            }
        }).appendTo(_elem);
