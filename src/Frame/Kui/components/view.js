function View(name, option) {
    ...
    this.render = function () {
        ...
        if (option.left) {
            var left = $('<div class="fit-col-3">').appendTo(elem);
            view = $('<div class="fit-col-7">').appendTo(elem);
            option.left.render().appendTo(left);
        } else if (option.inner) {
            view = elem;
        } else {
            view = $('<div class="fit-col">').appendTo(elem);
        }
    ...
    function _initGrid() {
        ...
        var gridOption = $.extend(true, {
            ...
            columns: _columns.slice().filter(function (d) { return !d.onlyForm && d.type !== 'hidden'; })
        }, _gridOption);
