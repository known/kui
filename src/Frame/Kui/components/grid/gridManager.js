...
function GridManager(grid, form, rows) {
    ...
    this.import = function (url, param) {
        ...
        Layer.open({
            ...
            content: function (dom) {
                var div = $('<div class="import-tips">').appendTo(dom);
    ...
    function loadXlsxCore() {
        if (!GridManager.isLoad) {
            Utils.addJs(baseUrl + '/static/libs/xlsx.core.min.js');
            GridManager.isLoad = true;
        }
    }
    ...
    function deleteData(e, url, callback) {
        ...
        Layer.confirm(text, function () {
            Ajax.post(url, { data: JSON.stringify(e.ids) }, function () {
                if (callback) {
                    callback(e);
                } else {
                    e.grid.reload();
