...
function FlowChart(canvasId, option) {
    ...
    this.render = function (elem) {
        var width = option.width || 1200;
        var height = option.height || 800;
        var font = option.font || 'normal 14px YaHei';
        $('<canvas id="' + canvasId + '" width="' + width + '" height="' + height + '">').appendTo(elem);
        ...
        for (var i = 0; i < option.items.length; i++) {
            var item = option.items[i];
            item.arrow && item.arrow({ step: _this.steps[i] });
        }
