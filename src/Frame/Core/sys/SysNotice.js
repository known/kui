function SysNotice() {
    ...
    var type = Utils.getUrlParam('type'),
        url = {
            QueryModels: moduleUrl + 'QueryNotices',
            DeleteModels: moduleUrl + 'DeleteNotices',
            SaveModel: moduleUrl + 'SaveNotice',
            PublishNotice: moduleUrl + 'PublishNotice'
        };
    ...
    var view = new View('SysNotice', {
        ...
        columns: [
            { field: 'Id', type: 'hidden' },
            {
                title: Language.Title, field: 'Title', query: true, format: function (d) {
                    return $('<span class="link">').append(d.Title).data('data', d).click(function () {
                        var data = $(this).data('data');
                        Layer.open({ title: data.Title, width: 960, height: 500, content: data.Content });
                    });
                },
        ...
        gridOption: {
            showModifyBy: true, showCreateBy: true, where: { type: type }
        }
    });
